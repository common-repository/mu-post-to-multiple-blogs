<?php
/*
Plugin Name: MU Post-to multiple blogs
Plugin URI: http://wordpress.org/plugins/mu-post-to-multiple-blogs
Description: Allows the posting to multiple blogs at a time, bubble-out style.  Must be logged in as network admin to see the option.
Version: 1.0
Author: Brian Dombrowski
Author URI: http://confluencecorp.com/
*/
/*  Copyright 2013 Confluence Corporation (http://confluencecorp.com)

	This plugin was based on the WordPress MU Site-wide plugin that Donncha O Caoimh made at http://ocaoimh.ie/wordpress-mu-sitewide-tags/
...
function swt_manager() {
	...
	$tags_blog_public = get_sitewide_tags_option( 'tags_blog_public', 0 );
	$tags_blog_pages = get_sitewide_tags_option( 'tags_blog_pages', 0 );
	$tags_blog_thumbs = get_sitewide_tags_option( 'tags_blog_thumbs', 1 );
	$tags_blog_pub_check = get_sitewide_tags_option( 'tags_blog_pub_check', 1 );
	$tags_blog_postmeta = get_sitewide_tags_option( 'tags_blog_postmeta', 0 );
	?>
	<?php /*  This section hidden b/c these params should not be changed.
	...
	<li><label><input name="tags_blog_main_blog" type="checkbox" id="tags_blog_main_blog" value="1" <?php if( get_sitewide_tags_option( 'tags_blog_main_blog', 1 ) == 1 ) { echo "checked='checked'"; } ?> /> <strong><?php _e( "Post to main blog","wpmu-sitewide-tags" ); ?></strong></label><br />
...
<?php } else { ?>
	...
	<?php /*  Populate feature hidden b/c no longer applicable with the post-to specific
...
function sitewide_tags_log($msg, $post_id, $post) {
	global $wpdb;

	$debug = 1;
	if ( $debug ) {
		$fh = fopen(plugin_dir_path(__FILE__).'sitewide-tags-log', 'a');
		fwrite($fh, $msg." - Post: ".$post_id." Blog: ".$wpdb->blogid."\n");
		fclose($fh); 
	}
}

function sitewide_add_meta_boxes() {
	function multi_post_meta_box( $post ) {

        //Example stuff, also see http://codex.wordpress.org/Function_Reference/add_meta_box
		//  global $wpdb,$post;
		...
		// Add an nonce field so we can check for it later, to verify this box existed
  		wp_nonce_field( 'multi_post_meta_box', 'multi_post_meta_box_nonce' );

		$multi_post_meta = unserialize(get_post_meta( $post->ID, 'multi_post_meta', true ));

		$blog_list = get_blog_list( 0, 'all' );
		// foreach ($blog_list as $blog) {
		// 	echo '<p>Blog '.$blog['blog_id'].': '.$blog['domain'].$blog['path'].'</p>';
		// }

		global $blog_id;
		echo "<div class='multi_post_meta_box'>";
		echo "<p>Select blogs to share this post with</p>";
		echo "<p><select name='multi_post_meta_select[]' multiple style='width:220px;'>";
		foreach($blog_list as $blog) {
			//Skip if $blog = current blog
			...
			//Select if selected before
			if(is_array($multi_post_meta) && in_array($blog['blog_id'],$multi_post_meta)) $selected="selected";
			else $selected="";

			echo "<option $selected value='{$blog['blog_id']}'>$blog_name</option>";	
		}
		echo "</select></p>";
		echo "</div>";
		...
		// echo "</pre>";
		// echo "<pre>";
		// var_dump($multi_post_meta);
		// echo "</pre>";
	}
	if(is_super_admin())
		add_meta_box('multi_post',__('Multi-Post'), 'multi_post_meta_box', 'post', 'side', 'high');
}
if( get_sitewide_tags_option( 'tags_blog_enabled' ) )
	add_action('add_meta_boxes', 'sitewide_add_meta_boxes');

function sitewide_save_post_meta_from_meta_box( $post_id ) {
	/*
	...
	*/
	...
	if ( ! isset( $_POST['multi_post_meta_box_nonce'] ) )
		return $post_id;

	$nonce = $_POST['multi_post_meta_box_nonce'];

	// Verify that the nonce is valid.
	if ( ! wp_verify_nonce( $nonce, 'multi_post_meta_box' ) )
		return $post_id;
	...
	$multi_post_meta_last = unserialize(get_post_meta( $post->ID, 'multi_post_meta', true ));

	// Sanitize user input.
	$multi_post_meta = isset( $_POST['multi_post_meta_select'] ) ? (array) $_POST['multi_post_meta_select'] : array();
	$multi_post_meta = array_map( 'intval', $multi_post_meta );


	// Update the meta field in the database.
	update_post_meta( $post_id, 'multi_post_meta', serialize($multi_post_meta) );

	return $multi_post_meta;
}
//This action called from sitewide_tags_post now
// if( get_sitewide_tags_option( 'tags_blog_enabled' ) )
// 	add_action( 'save_post', 'sitewide_save_post_meta_from_meta_box' );

$nesting_lock = false;  // This function calls itself when inserting posts on other blogs.  The nesting lock prevents post duplication and infinite loop
function sitewide_tags_post( $post_id, $post ) {
	global $wpdb;
	global $nesting_lock;

	if( !get_sitewide_tags_option( 'tags_blog_enabled' ) ) {
		sitewide_tags_log('Sitewide tags disabled', $post_id, $post);
		return;
	}

	if($nesting_lock) {
		sitewide_tags_log('Returning due to nesting lock', $post_id, $post);
		return;
	}
	$nesting_lock = true;

	// Delete existing children posts in all blogs.  The posts will actually be updated automagically, but if a blog was removed from the selection, this will catch it
	sitewide_tags_post_delete($post_id);

	// Update additional blog selections
	$multi_post_meta = sitewide_save_post_meta_from_meta_box($post_id);
	...
	if ( $blog_status != 1 && ( $blog_status != 0 || get_sitewide_tags_option( 'tags_blog_public') == 1 || get_sitewide_tags_option( 'tags_blog_pub_check') == 0 ) ) {
		sitewide_tags_log('Blog is not public', $post_id, $post);
		return;
	}
	...
	$post->post_thumbnail_id = get_post_thumbnail_id( $post_id );
	// sitewide_tags_log("post->post_thumbnail_id1 = $post->post_thumbnail_id", $post_id, $post);

	$post->guid = $post_blog_id . '.' . $post_id;
	sitewide_tags_log('Post has GUID - '.$post->guid, $post_id, $post);
	...
	if(is_array($multi_post_meta)) {
		foreach($multi_post_meta as $blog_id) {
			...
			if( is_array( $cats ) && !empty( $cats ) && $post->post_status == 'publish' ) {
				foreach( $cats as $t => $d ) {
					...
					sitewide_tags_log('Added category: '.$d['name'], $post_id, $post);

					/* Now get the category ID to be used for the post */
					$category_id[] = $wpdb->get_var( "SELECT term_id FROM " . $wpdb->get_blog_prefix( $blog_id ) . "terms WHERE slug = '" . $d['slug'] . "'" );
				}
			}

			$global_post = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->get_blog_prefix( $blog_id )}posts WHERE guid IN (%s,%s)", $post->guid, esc_url( $post->guid ) ) );
			//error_log("gp = ".print_r($global_post),0);
			if( $post->post_status != 'publish' && is_object( $global_post ) ) {
				wp_delete_post( $global_post->ID );
			} else {
				if( $global_post->ID != '' ) {
					sitewide_tags_log("gid=$global_post->ID");
			...
			if( $post->post_status == 'publish' ) {
				...
				// Set the post to draft so it can be published manually.
				//$post->post_status = 'draft';
				
				//Resolve cross posting issue
				$post->post_name = $post->post_name. '_'. $post_blog_id;
				...
				//wp_insert_post doesn't add the format or featured post at this time, so add it manually
				set_post_format($p, $post->post_format);
				set_post_thumbnail($p, $post->post_thumbnail_id); // This isn't working for some reason
				// sitewide_tags_log("post->post_thumbnail_id2 = ".get_post_thumbnail_id( $p ), $post_id, $post);
...
$nesting_lock2 = false;  // This function calls itself when inserting posts on other blogs.  The nesting lock prevents post duplication and infinite loop
function sitewide_tags_post_delete( $post_id ) {
	...
	$multi_post_meta = unserialize(get_post_meta( $post_id, 'multi_post_meta', true ));

	if(!is_array($multi_post_meta)) {
		sitewide_tags_log('multi_post_meta is not set', $post_id, $post);
		return;
	}
